...
Presenting facsimiles and text side-by-side for the validation of the latter with the help of the former is core to OpenArabicPE's claim of credibility. Therefore, and in order to make the editions accessible to readers with as little overhead as possible, we heavily modified the [TEI Boilerplate](http://dcl.slis.indiana.edu/teibp/) for the use with Arabic texts. Our adaptation of the TEI Boilerplate also provides access to a table of contents, limited browsing to neighbouring issues, and bibliographic metadata on the article level.[^17] By rendering XML files locally in a web browser on the reader's computer with XSLT and CSS, we remove the need for backend servers and internet connections. We added a simple system to provide multiple localisations and most parts of the interface have been translated to Arabic. {>>this is ongoing work<<} Editions can be downloaded, distributed through USB keys, and run locally. This also removes the need to produce reading surrogates whenever the main TEI XML was changed.

...
{>>add comment on neo-colonial absence of the GLobal South from the digital cultural record and the resulting epistemic violence. [@Risam+2019] Digitisation is costly, periodicals are considered of lesser importance, particularly in those in foreign languages. Cf. [@Gooding+2018]<<}
...
*Digitised* frequently means scanned facsimiles with limited or no text layer due, inter alia, to the state of Arabic OCR.[^7] Available text layers are frequently hidden. Vendors tend to foreground full-text "search" that won't ever report false negatives and verifiable information on error rates is either inexistent or not published.
